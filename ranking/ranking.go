...
func card_ranks(hand [5]string) ([5]int){
	var ranks [5]int
	rankSpecial := [5]int{14, 5, 4, 3, 2}
	for i, v := range hand {
	 	if (v[0] == 'A') {
			ranks[i] = 14
		} else if (v[0] == 'K') {
	...
	if (ranks  == rankSpecial) {
		ranks = [5]int{5, 4, 3, 2, 1}
	}
	return ranks
}

func hand_rank(hand [5]string) ([]int) {
...
func straight(ranks [5]int) (bool) {
	len := len(ranks)
...
func two_pairs(ranks [5]int) (int, int) {
	pair := kind(2, ranks)
...
func kind(n int, ranks [5]int) (rank int) {
	for _, v := range ranks {
		if (count(v, ranks) == n) {
			return v
		}
...
func count(r int, ranks [5]int) (int) {
	count := 0
	for _, v := range ranks {
		if (v == r) {
			count++
...
func findBestHand(cards []int) []int {
	...
	for i := 0; i < 6; i++ {
		for j := i + 1; j < 7; j++ {
			...
			if compareHands(curRank, maxRank) > 0 {		
				maxHand = curHand
			}
		}
	}
